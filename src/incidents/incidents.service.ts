...
export class IncidentsService {
  ...
  async getIncidentByRadius(
    ...
    try {
      logger.info(
        `Buscando incidentes en ${lat}, ${lon} en un radio de ${radius} m`,
      );
      const incidents = await this.incidentRepository
        ...
        .getMany();
      logger.info(
        `${incidents} incidentes encontrados en un radio de ${radius} m`,
      );
      return incidents;
    } catch (error) {
      console.error(error);
  ...
  async getIncidents(): Promise<Incident[]> {
    try {
      ...
      const data = await this.cacheService.get<Incident[]>(
      ...
      logger.info(
        `[IncidentService] Se obtuvieron ${incidents.length} incidentes`,
      );
      return incidents;
    } catch (error) {
